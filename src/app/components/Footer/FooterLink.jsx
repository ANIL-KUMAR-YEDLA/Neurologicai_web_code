        ...
        <div key={category} className="flex flex-col items-start gap-5">
          ...
          <ul className="space-y-2">
            {links[category].map(link => (
              <li key={link}>
                <Link
                  href={`/${category.toLowerCase()}/${link.toLowerCase().replace(/ /g, '-')}`}
                  className="text-[#979797] text-[13px] font-[Inter] font-normal leading-[26px] tracking-[-0.001px] hover:text-white transition"
                >
                  {link}
                </Link>
              </li>
